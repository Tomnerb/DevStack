...
cp -f bin/dockiva.exe dist/windows-amd64/dockiva.exe

echo
echo "=== macOS Apple Silicon ==="
wails3 build GOOS=darwin GOARCH=arm64
mkdir -p dist/macos-arm64
if [[ -f bin/dockiva ]]; then
  cp -f bin/dockiva dist/macos-arm64/dockiva
fi
...
cp -f bin/dockiva dist/linux-amd64/dockiva
